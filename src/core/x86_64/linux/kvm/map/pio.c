...
bool_t
    vp_pio_rd8
        (vp_map* par, u16_t par_pio, u8_t* par_buf)      {
            if (!par)                      return false_t;
            if (trait_of(par) != vp_map_t) return false_t;
            if (!par->ops.ops_pio)         return false_t;
            if (!par->ops.pio)             return false_t;

            return par->ops.ops_pio->rd8 (
                   par->ops.pio,
                   par_pio      ,
                   par_buf
            );
